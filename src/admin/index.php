...
<div id="overlay" style="width:300px;height:375px;-webkit-border-radius: 20px;-moz-border-radius: 20px;border-radius: 20px;border:2px solid #9C9C9C;background-color:#E8E8E8;">
    ...
    <div style="overflow:auto;height:285px;font-size:12px">
        <p><button onclick="window.location.replace('/admin/events.php'); return false">Edit Events</button></p>
        <p><button onclick="window.location.replace('/admin/users.php'); return false">Edit Users</button></p>
        <?php
        if($_SESSION['permissionLevel'] > 2) {
            echo '<p><button onclick="window.location.replace(\'/admin/permissions.php\'); return false">Edit Admin Permissions</button></p>';
        }
        ?>
    </div>
